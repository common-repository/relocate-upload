...
Creator URI: http://www.extremewebdesign.biz/relocate-upload/
*/

// all paths are relative to the server document home
//define('SERVER_DOC_ROOT', $GLOBALS['_SERVER']['DOCUMENT_ROOT']);
define('SERVER_DOC_ROOT', normalize_path(ABSPATH));

if( is_admin() ) {
	function ru_register_scripts() {
		//wp_enqueue_script( 'jquery' );
		//wp_enqueue_script( 'jquery-ui-draggable' );
		//wp_enqueue_script( 'jquery-ui-droppable' );
		wp_enqueue_script( 'jquery-ui-sortable' );
	}
	add_action( 'admin_enqueue_scripts', 'ru_register_scripts' );
...
function relocate_upload_js_action() {
	...
	if( !$ru_folders = get_option('relocate-upload-folders') ) {
		$ru_folders = array();
	}
	...
	if( file_exists($new_path) ) {
		...
		$new_guid = site_url() . $ru_folders[$folder]['path'] . basename($attachment_path);
...
function relocate_upload_library_filter( $where ) {
	...
	if ( strpos( $_SERVER['REQUEST_URI'], "/wp-admin/media-upload.php" ) === false && strpos( $_SERVER['REQUEST_URI'], "/wp-admin/upload.php" ) === false ) {
		return $where;
	}
...
function RU_admin_options() {
	...
	?>
	<style>
		#ru_list li:hover {cursor: move;}
		#ru_list li.fixed:hover {cursor: default;}
	...
	<?php
		...
		foreach( $ru_folders as $ru_folder ) {
			$new_dir = normalize_path ( SERVER_DOC_ROOT . replace_month_year( $ru_folder['path'], date("Y-m") ) );
			// create directory if needed
			if( !file_exists( $new_dir ) ) {
				wp_mkdir_p( $new_dir );
			}
			$bad_folder = ( $disabled == "" && $new_dir && !is_writable( $new_dir ) ) ? " class='bad_folder ui-state-default'" : " class='ui-state-default'";
			if ( ( $ru_folder['name'] == 'Default location' ) || ( $ru_folder['path'] == "" ) ) {
				$bad_folder = " class='fixed'";
			}
			echo "<li $bad_folder >";
			echo "<input name='ru_folder_name[]' type='text' value='" . $ru_folder['name'] . "' " . $disabled." style='width:20%' />";
			echo "<input name='ru_folder_path[]' type='text' value='" . $ru_folder['path'] . "' " . $disabled." style='width:70%' />";
	...
	<script>
		jQuery("#ru_list span.remove").click(function(){
			jQuery(this).parent().remove();
		});
		jQuery(function() {
			jQuery("#ru_list").sortable({items: "li:not('.fixed')"});
		});
	</script>
	<?php
...
// replace all slashes with forward slashes and remove double slashes
function normalize_path( $path ) {
	$path = str_replace( array( '/', '\\' ), '/', $path );	// replace all slashes with forward slashes
	$path = str_replace( array( '//', '/' ), '/', $path );  // remove double slashes
	return $path;
}
